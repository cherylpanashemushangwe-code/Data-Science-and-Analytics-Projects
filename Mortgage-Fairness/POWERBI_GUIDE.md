...
- Apply **conditional background color** on Disparity Ratio:
  - Rules: > 1.25 → Red, 0.9–1.25 → Yellow, < 0.9 → Green
...
### Color Theme
Apply a custom theme — **View → Themes → Customize**:
- Primary: `#1F77B4` (blue)
- Alert: `#D62728` (red)
...
- Add a **Text Box** header on each page:
  `FairLend Analytics | HMDA 2023 | [State] | CFPB Fair Lending Framework`
- Add your name and date in the footer

### Page Navigation Buttons
- Insert → Buttons → Page Navigator
- Place at the top of every page for easy navigation

---

## Step 5 — Publish & Share

1. **File → Publish → Publish to Power BI** (requires free Power BI account)
2. Share the link or export as PDF (**File → Export → PDF**)
3. For interviews: export each page as PNG (**right-click canvas → Copy as image**)
...
| Question | Answer |
|----------|--------|
| *"Why Power BI over Tableau?"* | Power BI integrates natively with Azure and Microsoft stack — the standard at most large US banks |
| *"How did you get the data?"* | CFPB HMDA public API — same data regulators use for fair lending exams |
| *"What does the 1.25x mean?"* | CFPB examination standard — denial rate more than 25% higher than White applicants triggers a fair lending review |
| *"What's the business impact?"* | Banks with disparities like this face consent orders averaging $30M–$100M in settlements |
